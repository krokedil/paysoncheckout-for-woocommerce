...
class PaysonCheckout_For_WooCommerce_Get_Recurring_Payment extends PaysonCheckout_For_WooCommerce_Request {
	...
	public function request( $payment_id = null ) {
		if ( null !== $payment_id ) {
			...
			// Log the request.
			$log = PaysonCheckout_For_WooCommerce_Logger::format_log( $payment_id, 'GET', 'Payson get recurring payment request.', $request_args, $request_url, json_decode( wp_remote_retrieve_body( $response ), true ), $code );

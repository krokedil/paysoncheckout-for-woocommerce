...
export class PaysonIFrame {
    ...
    async finishOrder() {
        await Promise.all([
            this.page.waitForResponse(response => response.url().includes('pco_wc_update_checkout') && response.status() === 200), //Is this needed?
            this.iframe.locator('#SubmitAddress').click()
        ]);
        
        await Promise.all([
            this.page.waitForResponse(response => response.url().includes('pco_wc_update_checkout') && response.status() === 200), //Is this needed?
            this.iframe.getByRole('radio', { name: 'Bank account' }).click()
        ]);

...
const completeOrder = async (page, kcoIframe) => {
	...
	if (fullscreenIframe) {
		if (
			await fullscreenIframe.$("[data-cid='payment-selector-method.invoice']")
		) {
			await fullscreenIframe.click(
				'[data-cid="payment-selector-method.invoice"]'
			);
		}
		...
		if (await fullscreenIframe.$("[id='nin']")) {
			...
			await page.waitForTimeout(0.25 * timeOutTime);
			if (
				await fullscreenIframe.$('[id="b2b_nin_dialog__footer-button-wrapper"]')
			) {
				await fullscreenIframe.click(
					'[id="b2b_nin_dialog__footer-button-wrapper"]'
				);
			}
		}
...
const getOrderData = async (thankyouIframe) => {
	...
	let data = await thankyouIframe.$eval(selector, (element) => {
		...
		for (let i = 0; i < orderLineData.childNodes.length; i++) {
			if (
				orderLineData.childNodes[i].getElementsByTagName("p").length > 0 &&
        orderLineData.childNodes[i].firstChild.innerHTML !== "Delivery"
			) {
				orderLines++;
			}
		}

	...
	private function get_order_lines_from_order( $order_id ) {
		...
		if ( sizeof( $order->get_items() ) > 0 ) {
			foreach ( $order->get_items() as $item_key => $item ) {
				...
				$payData->AddOrderItem( new  PaysonEmbedded\OrderItem( $title, $price, $qty, $vat, $sku, PaysonEmbedded\OrderItemType::PHYSICAL, 0) );
			}
		}
		...
		if ( WC()->shipping->get_packages() ) {
			foreach ( WC()->shipping->get_packages() as $shipping_package ) {
				foreach ( $shipping_package['rates'] as $shipping_rate_key => $shipping_rate_value ) {
					...
					$qty   = 1;
					$sku   = 'Shipping';
					$payData->AddOrderItem( new  PaysonEmbedded\OrderItem( $title, $price, $qty, $vat_percent, $sku, PaysonEmbedded\OrderItemType::PHYSICAL, 0 ) );
				}
			}
		}
		// Process fees
